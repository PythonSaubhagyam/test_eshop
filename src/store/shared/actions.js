...
import { removeLocalToken, setLocalToken } from './lib/localStorage';

const requestProduct = () => ({ type: t.PRODUCT_REQUEST });

export const receiveProduct = (product) => ({ type: t.PRODUCT_RECEIVE, product });
...
export const fetchCart = (token) => async (dispatch, getState) => {
	dispatch(requestCart());
	const response = await api.ajax.cart.retrieve(token);
...
export const addCartItem = (item) => async (dispatch, getState) => {
	...
	const token = get(state, 'app.customerProperties.token');
	const response = await api.ajax.cart.addItem({ ...item, token });
	const cart = response.json;
	dispatch(receiveCart(cart));
...
export const preCheckCart = (cart, history) => async (dispatch, getState) => {
	// dispatch(requestCart());
	const response = await api.ajax.cart.client.put('/cart/check_items');
	const order = response.json;
	// dispatch(receiveCart(order));
};
...
const receiveAddressStates = (states) => ({ type: t.RECEIVE_STATES, states });
...
export const logoutUser = () => async (dispatch, getState) => {
	console.log('logout');
	Lscache.remove('auth_data');
	await api.ajax.logout.retrieve();
...
export const getstateCodes = (country_code, callback) => async (dispatch, getState) => {
	const response = await api.ajax.account.client.get(`/customer-account/address/${country_code}/states`);
	dispatch(receiveAddressStates(response.json));
};
