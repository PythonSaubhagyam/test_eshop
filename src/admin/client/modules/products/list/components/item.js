...
const ItemImage = ({ image }) => {
	if (image) {
		const imageUrl = image.url.replace('stage', 'cdn2');
...
const ItemStock = ({ status, quantity }) => {
	...
	return <div className={stockClass}>{quantity}</div>;
};
	...
	return (
		<div className={'products-item' + (selected === true ? ' selected' : '')}>
			<div className={'row row--no-gutter middle-xs ' + style.innerItem}>
				...
				<div className={'col-xs-2 ' + style.sku}>{product.product_code}</div>
				<div className={'col-xs-2 ' + style.stock}>
					<ItemStock status={product.stock_status} quantity={product.available_quantity} />
				</div>
