							...
							.map((k, i) => {
								return (
									<th className="td-header" key={keyCounter + i}>
										<div
											ref="status"
											className={k === 'state' ? 'sheet-cell-state' : 'sheet-cell-' + i}
				...
				<div style={{ width: '100%' }}>
					<div className="spread-sheet-container" style={this.state.productsImport}>
						<div style={{ margin: 20, color: 'rgba(0, 0, 0, 0.52)' }}>
							...
							<p>
								{' '}
								{messages.settings_googlesheet_products} {this.state.product_items.length - 1} /{' '}
								{messages.settings_googlesheet_uploaded} {this.state.uploadedProducts}
								{this.state.errors > 0 ? '/ ' + messages.settings_googlesheet_errors + ' ' + this.state.errors : ''}
